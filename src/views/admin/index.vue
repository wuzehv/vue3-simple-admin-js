<template>
    <div class="brisk-main">
        ...
        <div class="brisk-search-box" v-if="showSearch">
            <el-form ref="searchForm" :model="searchForm" :rules="searchRules" size="default" label-width="100px" label-position="left" style="margin-bottom: 20px">
                <el-row :gutter="20">
        ...
        <!-- 新增弹窗 -->
        <briskDialogcom :title="$t('admin.component.addlog_add_title')" :device="$store.state.app.device" :showDialog="addDialogFormVisible" @cancel="addCancel" @submit="addSubmit" @close="addCancel">
            <template #form>
                <el-form ref="addForm" :model="addForm" label-width="80px" label-position="left" size="default">
                    ...
                    <el-form-item :label="$root.$t('admin.field.status')">
                        <el-radio-group v-model="addForm.status">
                            <el-radio v-for="item in statusFilters" :key="item.value" :value="item.value">{{$root.$t('admin.field.'+item.label)}}</el-radio>
                        </el-radio-group>
                    </el-form-item>
                </el-form>
            </template>
        ...
        <!-- 详情弹窗组件 -->
        <briskDialogcom :title="$t('admin.component.addlog_detail_title')" :device="$store.state.app.device" :showDialog="detailDialogFormVisible" @cancel="detailCancel" @submit="detailSubmit" @close="detailCancel">
            <template #form>
        ...
        <!-- 编辑弹窗组件 -->
        <briskDialogcom :title="$t('admin.component.addlog_edit_title')" :device="$store.state.app.device" :showDialog="editDialogFormVisible" @cancel="editCancel" @submit="editSubmit" @close="editCancel">
            <template #form>
                <el-form ref="editForm" :model="editForm" label-width="80px" label-position="left" size="default">
                    <el-form-item :label="$root.$t('admin.field.username')">
                    ...
                    <el-form-item :label="$root.$t('admin.field.status')">
                        <el-radio-group v-model="editForm.status">
                            <el-radio v-for="item in statusFilters" :value="item.value">{{$root.$t('admin.field.'+item.label)}}</el-radio>
                        </el-radio-group>
                    </el-form-item>
                </el-form>
            </template>
