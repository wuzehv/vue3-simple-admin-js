    ...
    <el-dialog :title="title" :top="top" :fullscreen="fullscreen?fullscreen:fullcomd" :width="width" :model-value="showDialog" :destroy-on-close="destroyOnClose" @close="$emit('close')">
        ...
        <template #footer>
            <span class="dialog-footer">
                <slot name="btn"></slot>
                  <el-button v-if="showBtn.submit" type="primary" @click="$emit('submit')">{{$root.$t("dialogcom.confirm")}}</el-button>
                <el-button v-if="showBtn.cancel" @click="$emit('cancel')">{{$root.$t("dialogcom.cancel")}}</el-button>
            </span>
        </template>
    </el-dialog>
</template>
